...
all_img = 0
def generate_image(DsAndGs, itr, reals, loss_dict1, loss_dict2 = None,
                   fakes = None, oToPickle = None):
    """
    ...
    if ntbk:
        ...
        plt.savefig(op_folder + '/' + 'frame' + str(all_img) + '.jpg')
        plt.close()
        for fname in loss_dict1:
            with open(op_folder+"/"+fname+".pkl", "wb") as f:
        ...
        if itr >= 100:
            ...
            if oToPickle is not None:
                torch.save(oToPickle
                       , op_folder+"/oToPickle_"+str(itr)+".mdl")
    all_img += 1
...
s_style_noise = torch.randn(opt.batchSize, opt.nz)
mse = nn.MSELoss()
if cuda:
    preG = preG.cuda(gpu)
        ...
        if iters%5000 == 0 or iters in disp_itrs:
            ...
            for d_i in range(2):
                ...
                with torch.no_grad():
                    ...
                    fixed_noise[d_i][:opt.hm_gen_size] =                     preG(HM[d_i]).data
